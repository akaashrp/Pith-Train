...
class SourceDataset:
    """
    A named corpus source composed of one or more MemmapDatasets.
    """
    ...
    def __init__(self, name: str, memmap_datasets: List[MemmapDataset]):
        ...
        self.total_len = int(self.offsets[-1]) if len(self.offsets) > 0 else 0
        if self.total_len <= 0:
...
class WeightedMixtureDataset:
    ...
    Sampling is deterministic per sample index and seed, so all ranks can
    independently fetch samples without sharing dataset state.
    """
    ...
    def update_weights(self, weights: Dict[str, float]) -> None:
        ...
        # Guard against tiny FP drift when total is mathematically 1.0.
        self._cdf[-1] = 1.0
    ...
    def __len__(self) -> int:
        # Informational only: weighted sampling is with replacement.
